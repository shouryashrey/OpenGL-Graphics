...
using namespace std;
float boundaryColor[] = {0, 0.5, 1}; //boundary color of our polygon
float fillColor[] = {0, 0.5, 1};     //color to be filled inside the polygon
...
void draw_poly()
{
    ...
    glColor3fv(boundaryColor); //passing the boundaryColor vector to set color of polygon
...
void boundary_fill8(int x, int y, float *fillColor, float *boundaryColor)
{
    ...
    //bc is boundary color, fillColor is color with which we wish to fill our polygon
    ...
    if ((color[0] != boundaryColor[0] || color[1] != boundaryColor[1] || color[2] != boundaryColor[2]) && (color[0] != fillColor[0] || color[1] != fillColor[1] || color[2] != fillColor[2]))
    {
        ...
        //recursive call for 4 diagonal neighbours: Upper-Left, Upper-Right, Down-Left, Down-Right
